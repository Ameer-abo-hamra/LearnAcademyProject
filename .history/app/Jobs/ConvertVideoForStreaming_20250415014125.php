...
class ConvertVideoForStreaming implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // $videoPath يجب أن تكون القيمة بالنسبة لجذر القرص المُستخدم
        $videoPath = $this->video->path; // تأكد أن هذه القيمة مثل "1/1/79.mp4"
        $disk = 'stremable_video';

        // المسار الذي سيتم حفظ ملفات HLS فيه (داخل مجلد مخصص)
        $basePath = $this->video->teacher_id . '/' . $this->video->course_id . '/' . $this->video->id;

        // إعداد FFmpeg، ولا تقم بتعديل $videoPath
        $hls = \ProtoneMedia\LaravelFFMpeg\Support\FFMpeg::fromDisk($this->video->disk) // مثلاً 'teachers'
            ->open($videoPath)
            ->exportForHLS()
