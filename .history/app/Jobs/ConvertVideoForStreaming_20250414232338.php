...
class ConvertVideoForStreaming implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $relativePath = 'videos/' . $this->video->teacher_id . '/' . $this->video->course_id . '/' . $this->video->id;

        // تحديد المسار الذي سيتم حفظ الفيديوهات المحولة فيه
        $basePath = public_path('uploads/' . $this->video->teacher_id . '/' . $this->video->course_id . '/' . $this->video->id);
        ...
        $hls = \ProtoneMedia\LaravelFFMpeg\Support\FFMpeg::fromDisk($disk)
            ->open($videoPath)
            ->exportForHLS()
            ->toDisk('teachers');  // تأكد من أن الحفظ في public path
