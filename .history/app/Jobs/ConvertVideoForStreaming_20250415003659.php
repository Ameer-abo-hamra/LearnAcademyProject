...
class ConvertVideoForStreaming implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // مسار الفيديو الأصلي من الـ database (أنت بتدخله يدويًا)
        ...
        $hls = \ProtoneMedia\LaravelFFMpeg\Support\FFMpeg::fromDisk($this->video->disk) // الديسك الذي رفع عليه الفيديو الأصلي
            ->open($videoPath.'/'.$this->video->teacher_id.'/'.$this->video->course_id)
            ->exportForHLS()
            ->toDisk($disk) // التخزين في stremable_video
            ->setSegmentLength(10) // طول كل قطعة .ts
            // ->useSegmentFilename('%03d.ts')
        ; // تسمية الملفات داخل كل دقة
        ;
        // إضافة كل دقة
        ...
        // حفظ ملف m3u8 الرئيسي داخل المسار
        try {
            ...
            throw $e; // لو تحب توقف التنفيذ وتخلي Laravel يرمي الخطأ.
        }
