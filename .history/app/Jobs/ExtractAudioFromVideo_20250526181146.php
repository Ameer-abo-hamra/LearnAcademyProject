        ...
        try {
            ...
            if ($status === 'completed') {
                ...
                if ($resultResponse->successful()) {
                    ...
                    $subDir = "{$video->course->teacher_id}/{$video->course_id}/{$video->id}";
                    ...
                    if (!empty($transcription)) {
                        ...
                        $vttName = "{$subDir}/{$lang}.vtt";
                        $vttContent = $generateVttContent($transcription);

                        Storage::storeAs('video_subTitle')->put($vttName, $vttContent);

                        $video->videoSubtitles()->create([
                            'language' => $lang,
                            'path' => asset("video_subTitle/{$vttName}"),
                        ]);
                    }
                    ...
                    foreach ($translations as $lang => $data) {
                        if (!empty($data['segments'])) {
                            ...
                            Storage::disk('video_subTitle')->put($vttName, $vttContent);
