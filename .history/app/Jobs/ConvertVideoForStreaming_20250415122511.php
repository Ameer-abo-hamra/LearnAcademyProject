...
class ConvertVideoForStreaming implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // استخدم قيمة المسار المخزنة في قاعدة البيانات، تأكد أنها نسبية مثل "1/1/79.mp4"
        $videoPath    = $this->video->path;
        $diskOriginal = $this->video->disk;  // مثلاً "teachers"
        $diskOutput   = 'stremable_video';     // للملفات الناتجة

        // بناء مسار الدليل للإخراج باستخدام DIRECTORY_SEPARATOR لضمان الفواصل الصحيحة
        $basePath = $this->video->teacher_id . DIRECTORY_SEPARATOR .
                    $this->video->course_id . DIRECTORY_SEPARATOR .
                    $this->video->id;

        // التأكد من وجود الدليل على القرص المخصص للإخراج
        ...
        // تسجيل القيم
        \Log::info('Disk Original: ' . $diskOriginal);
        \Log::info('Video Path (relative): ' . $videoPath);
        \Log::info('Full input path: ' . public_path('uploads/' . $videoPath));
        \Log::info('Output base path: ' . $basePath);
        \Log::info('Full save path: ' . $savePath);
        ...
        // إعداد FFmpeg باستخدام المسار والمخرجات المحددة
        ...
        // إضافة تنسيقات الفيديو المطلوبة
