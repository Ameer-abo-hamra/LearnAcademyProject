...
class StudentController extends Controller
{
    ...
    public function searchCoursesAndSpecializations(Request $request)
    {
        ...
        // 📚 جلب الكورسات
        $coursesPaginated = Course::when($filters['course_name'] ?? null, fn($q, $v) => $q->where('name', 'like', "%$v%"))
        ...
        // 🧠 التخصصات
        $specializationsPaginated = Specilization::when($filters['specialization_title'] ?? null, fn($q, $v) => $q->where('title', 'like', "%$v%"))
            ->when($filters['is_completed'] ?? null, fn($q, $v) => $q->where('is_completed', $v))
            ->orderBy($sortBy, $sortDir)
            ->paginate($perPage);

        // 🎁 تفريغ النتائج بدون بيانات التصفح (نستخدم فقط الـ collection)
        $courses = $coursesPaginated->getCollection();
        $specializations = $specializationsPaginated->getCollection();

        // 🔢 إعداد الميتا (نرسل الاثنين للـ returnData)
        $meta = [
            $coursesPaginated,
            $specializationsPaginated
        ];
