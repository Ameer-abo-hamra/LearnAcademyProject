...
class CourseController extends Controller
{
    ...
    public function makeCourse(Request $request)
    {
        ...
        try {
            ...
            ])->post("http://localhost:8000/your-endpoint", [
                        'course_id' => (string) $course->id,
                        'value' => $request->skills, // أو أي قائمة أخرى مناسبة
                    ]);

            if ($response->successful()) {
                // يمكنك تسجيل النجاح مثلاً:
                Log::info("✅ External course data sent successfully", $response->json());
            } else {
                Log::error("❌ Failed to send external request", [
