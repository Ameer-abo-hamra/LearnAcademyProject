...
class StudentController extends Controller
{
    ...
    public function searchCoursesAndSpecializations(Request $request)
    {
        ...
        // 📦 الكورسات
        $courses = Course::when($filters['course_name'] ?? null, fn($q, $v) => $q->where('name', 'like', "%$v%"))
        ...
        $courses = $courses->getCollection();
        $specializations = $specializations->getCollection();
        $data = [
            'courses' => $courses,
            'specializations' => $specializations
        ];
        return $this->returnData("results", $data, 200, $meta);
    }
