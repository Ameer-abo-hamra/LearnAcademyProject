...
class VideoController extends Controller
{
    ...
    public function getSubTitles($video_id, $lang)
    {
      ...
      public function getAudioFiles($videoId)
    {
        $video = Video::with('audios')->find($videoId);

        if (!$video) {
            return $this->returnError('Video not found');
        }

        $audios = $video->audios->map(function ($audio) {
            return [
                'id' => $audio->id,
                'path' => $audio->path,
        ...
        return $this->returnSuccess('Audio files fetched successfully', 200, [
            'video_id' => $video->id,
            'audios' => $audios,
        ]);
    }
