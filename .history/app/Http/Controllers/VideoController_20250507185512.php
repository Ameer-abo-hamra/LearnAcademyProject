...
class VideoController extends Controller
{
    ...
    public function watchVideoForStudent($video_id)
    {
        ...
        $alreadyWatched = $student->watchedVideos()->where('video_id', $video->id)->exists();
        if (!$alreadyWatched) {
            $student->watchedVideos()->attach($video->id, ['watched_at' => now()]);
        }
    ...
    public function completeVideo($video_id)
    {
        ...
        // فتح الفيديو التالي (إن وجد)
        $nextVideo = Video::where('course_id', $video->course_id)
            ->where('sequential_order', "=", $video->sequential_order + 1)
            ->first();

        if ($nextVideo) {
            $nextVideo = StudentCourseVideo::where('student_id', $student->id)
                ->where('video_id', $nextVideo->id)->first()
            ;
            if (!$nextVideo->locked) {
                return $this->returnError("this video is alreadey wateched");
            }
            $nextVideo->update(['locked' => false]);
        }
    ...
    public function getCoursePrecentage($course_id)
    {
        ...
        return $this->returnData("progress", $percentage->per);
    }
