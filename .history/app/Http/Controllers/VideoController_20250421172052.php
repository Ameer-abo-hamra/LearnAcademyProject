...
class VideoController extends Controller
{
    ...
    public function addExtension(Request $request, $videoId)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'file.required_without' => 'يجب رفع ملف PDF أو كتابة نصّ كامتداد.',
            'text.required_without' => 'يجب كتابة نصّ أو رفع ملف PDF كامتداد.',
        ]);
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                ...
                $folderPath = "{$video->teacher_id}/{$video->course_id}/{$video->id}";
                $extFilePath = $request->file('file')->store($folderPath, 'video_extension');
            }
            ...
            DB::commit();
            return $this->returnSuccess('تم إضافة الامتداد بنجاح.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function addQuestions(Request $request, $videoId)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'questions.min'         => 'يجب أن تحتوي على سؤال واحد على الأقل.',
            'questions.*.time_to_appear.date_format' => 'تنسيق الوقت يجب أن يكون H:i:s.',
        ]);
        ...
        try {
            ...
            DB::commit();
            return $this->returnSuccess('تم إضافة الأسئلة بنجاح.');
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->returnError($e->getMessage(), 500);
        }
