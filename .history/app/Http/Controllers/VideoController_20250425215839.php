...
class VideoController extends Controller
{
    ...
    public function getCourseVideo( Request $request, $, $video_id)
    {
        $course_id = $request->query("course_id");
        $video_id = $request->query("course_id");
        $teacherCourses = u("teacher")->courses();
        $course = $teacherCourses->find($course_id)->first();
        $video = $course->videos()->where("id", $video_id);
        return $this->returnData("", $video, 200);
