        ...
        $validator->after(function ($validator) use ($request) {
            ...
            if ($request->has('attachments')) {
                foreach ($request->attachments as $index => $attachment) {
                    if (empty($attachment['file']) && empty($attachment['text'])) {
                        $validator->errors()->add("attachments.$index", 'Each attachment must have at least a file or a text.');
                    }
                }
            }
        });
            ...
            if ($request->has('attachments')) {
                foreach ($request->attachments as $attachment) {
                    ...
                    if (isset($attachment['file'])) {
                        ...
                        $folder_name = $course->teacher->id . '/' . $course->id . '/' . '.' . $uploadedFile->getClientOriginalExtension();
                        $filePath = $uploadedFile->storeAs("course_attachments", $file_name, 'course_attachments'); // أو غيّر 'public' لو عندك ديسك آخر
                    }
