...
class UserController extends Controller
{
    ...
    public function rejectCourse($courseId)
    {
        try {
            ...
            if ($course->status != 1) {
                return $this->returnError("Only pending courses can be rejected.");
            }

            $course->status = 3; // نفترض أن 3 تعني "مرفوض"
            $course->save();

            return $this->returnSuccess("Course has been rejected successfully.");
        } catch (\Exception $e) {
            return $this->returnError("Failed to reject course: " . $e->getMessage());
        }
    }
