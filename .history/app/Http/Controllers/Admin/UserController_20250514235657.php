...
class UserController extends Controller
{
    ...
    {
        try {
            $spec = Specilization::with(['courses.teacher']) // نجلب الكورسات ومعها اسم الأستاذ
                ->find($specialization_id);
            ...
            $courses = $spec->courses->map(function ($course) use ($spec) {
                return [
                    'id' => $course->id,
                    'name' => $course->name,
                    'description' => $course->description,
            ...
            $courses->specialization =$spec ;

            return $this->returnData('Courses retrieved successfully.', $courses);
