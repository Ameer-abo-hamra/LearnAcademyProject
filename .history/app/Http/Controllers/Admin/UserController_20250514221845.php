    ...
    public function updateSpecializationByAdmin(Request $request, $id)
{
    ...
    try {
        if ($request->has('title')) $spec->title = $request->title;
        if ($request->has('is_completed')) $spec->is_completed = $request->is_completed;
        if ($request->has('teacher_id')) $spec->teacher_id = $request->teacher_id;
