    ...
    public function getStudents(Request $request)
{
    try {
        $students = Student::select([
                'id', 'full_name', 'email', 'username', 'activation_code', 'age',
                'image', 'gender', 'free_points', 'paid_points', 'is_active',
                'admin_activation', 'created_at', 'updated_at'
            ])
            ->orderBy('created_at', 'desc')
            ->paginate(10); // يمكنك تغيير العدد حسب الحاجة

        return $this->returnData('Students retrieved successfully', $students);
    } catch (\Exception $e) {
...
public function getTeachers(Request $request)
{
    try {
        $teachers = Teacher::select([
                'id', 'full_name', 'email', 'username', 'activation_code', 'age',
                'image', 'gender', 'specialization', 'is_active', 'admin_activation',
                'created_at', 'updated_at'
            ])
            ->orderBy('created_at', 'desc')
            ->paginate(10); // يمكنك تعديل العدد حسب الطلب

        return $this->returnData('Teachers retrieved successfully', $teachers);
    } catch (\Exception $e) {
