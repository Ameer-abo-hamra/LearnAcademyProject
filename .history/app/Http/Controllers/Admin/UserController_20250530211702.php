...
class UserController extends Controller
{
    ...
    public function adminLogin(Request $request)
    {
        ...
        try {
            ...
            if ($token) {
                ...
                return $this->returnData('Login successful', $admin, 200);
            }
        } catch (\Exception $e) {
            return $this->returnError($e->getMessage());
        }

        return $this->returnError('Your credentials are invalid');
    }
    ...
    public function rejectCourse(Request $request, $course_id)
    {
        ...
        try {
            ...
            Notification::create([
                'notifiable_id' => $course->teacher->id,
                'notifiable_type' => \App\Models\Teacher::class,
                'sender_id' => u('admin')->id, // أو auth()->id() حسب نظام التوثيق
                'sender_type' => \App\Models\Admin::class,
                'title' => $message['title'],
                'body' => $message["reason"],
                'data' => json_encode([
    ...
    public function getCoursesBySpecialization($specialization_id)
    {
        try {
            $spec = Specilization::with(['courses.teacher'])->find($specialization_id);
            ...
            $specializationData = [
                ...
                'teacher' => $spec->teacher ? [
                    'id' => $spec->teacher->id,
                    'full_name' => $spec->teacher->full_name,
                ] : null,
            ];

            // بيانات الكورسات المرتبطة
            $courses = $spec->courses->map(function ($course) {
                return [
                    'id' => $course->id,
                    'name' => $course->name,
                    'description' => $course->description,
            ...
            return $this->returnData('Courses retrieved successfully.', [
                'specialization' => $specializationData,
                'courses' => $courses
            ]);

        } catch (\Exception $e) {
            return $this->returnError('Failed to load courses: ' . $e->getMessage(), 500);
        }
    }
    ...
    public function getQuizForAdmin($quiz_id)
    {
        $quiz = Quize::with([
            ...
            'questions' => function ($query) {
                $query->select('id', 'text', 'quiz_id'); // تحديد الأعمدة
            }
        ])->find($quiz_id);
