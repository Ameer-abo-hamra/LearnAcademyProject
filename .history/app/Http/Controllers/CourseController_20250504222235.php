...
class CourseController extends Controller
{
    ...
    public function getCourseForStudent($course_id)
    {
        ...
        $student = u("student");
        $StId = $student->id;
        $is = $student->courses()->where($StId, "student_id")->where($course_id, "course_id")->exists();
        if (!$is) {
            return $this->returnError("you cant see this course .. enroll first :(");
        }

        $course = Course::where($course_id , "");
        $firstCourse = [
            "name" => $course->name,
            "status" => $course->status,
            "description" => $course->description,
