...
class StudentController extends Controller
{
    ...
    {
        // 1. اجلب المدخلات الأساسية
        $q          = $request->input('q');            // الكلمة العشوائية
        $minPoints  = $request->input('min_points');   // أقل قيمة لنقاط التسجيل
        $maxPoints  = $request->input('max_points');   // أعلى قيمة
        $level      = $request->input('level');        // مستوى الدورة
        $isCompleted= $request->input('is_completed'); // تخصص مكتمل؟ 0 أو 1
        ...
        $courses = Course::query()
            ...
            ->when($minPoints, function($query, $minPoints) {
                $query->where('point_to_enroll', '>=', $minPoints);
            })
            ->when($maxPoints, function($query, $maxPoints) {
                $query->where('point_to_enroll', '<=', $maxPoints);
            })
        ...
        // 3. بحث في التخصصات
        $specializations = spe::query()
            ->when($q, function($query, $q) {
                $query->where('title', 'like', "%{$q}%");
            })
            ->when(!is_null($isCompleted), function($query) use ($isCompleted) {
