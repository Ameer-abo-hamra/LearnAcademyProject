...
class StudentController extends Controller
{
    ...
    public function searchCoursesAndSpecializations(Request $request)
    {
        ...
        $filters = $request->only([
            'course_name', 'course_level', 'course_status',
            'min_points_to_enroll', 'max_points_to_enroll',
            'min_points_earned', 'max_points_earned',
            'specialization_title', 'is_completed',
            'sort_by', 'sort_direction', // للترتيب
            'per_page' // للتقسيم
        ]);

        $sortBy = $filters['sort_by'] ?? 'created_at';
        $sortDir = in_array(strtolower($filters['sort_direction'] ?? 'desc'), ['asc', 'desc']) ? $filters['sort_direction'] : 'desc';
        $perPage = is_numeric($filters['per_page'] ?? null) ? (int) $filters['per_page'] : 10;

        // 📦 الكورسات
        $courses = Course::with(['teacher', 'category', 'specilization'])
        ...
        // 📦 التخصصات
        $specializations = Specilization::with('courses')
            ->when($filters['specialization_title'] ?? null, fn($q, $v) => $q->where('title', 'like', "%$v%"))
            ->when($filters['is_completed'] ?? null, fn($q, $v) => $q->where('is_completed', $v))
            ->orderBy($sortBy, $sortDir)
            ->paginate($perPage);
