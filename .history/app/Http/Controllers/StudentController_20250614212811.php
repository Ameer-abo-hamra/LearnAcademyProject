...
public function sendRecomendation(Request $request)
{
    // تحقق من وجود حقل interest كـ string
    $request->validate([
    ...
    try {
        ...
        ])->post('http://localhost:8005/api/v1/courses/search', $payload);

        // معالجة الرد
        if ($response->successful()) {
            return $this->returnSuccess('Recommendations fetched successfully', 200, $response->json());
        } else {
            return $this->returnError('Failed to fetch recommendations. Status: ' . $response->status());
        }
    } catch (\Exception $e) {
