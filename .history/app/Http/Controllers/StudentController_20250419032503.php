...
class StudentController extends Controller
{
    ...
    public function searchCoursesAndSpecializations(Request $request)
    {
        ...
        // 📦 الكورسات
        $coursesPaginated = Course::with(['teacher', 'category', 'specilization'])
            ->when($courseFilters['name'] ?? null, fn($q, $v) => $q->where('name', 'like', "%$v%"))
        ...
        // 📦 التخصصات
        $specializationsPaginated = Specilization::with('courses')
            ->when($specializationFilters['title'] ?? null, fn($q, $v) => $q->where('title', 'like', "%$v%"))
