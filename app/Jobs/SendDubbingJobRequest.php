...
class SendDubbingJobRequest implements ShouldQueue
{
    ...
    public function __construct($filePath, $transcriptionJson, $targetLanguage = 'en', $outputFormat = 'mp3', $videoId)
    {
    ...
    public function handle()
    {
        ...
        if (strtolower($status) === 'completed') {
            ...
            if ($result->successful()) {
                ...
                if ($video) {
                    $relativeDubbedPath = str_replace(public_path(), '', $outputFilePath);
                    $relativeDubbedPath = str_replace('\\', '/', $relativeDubbedPath); // ✅ إصلاح الباك سلاش
